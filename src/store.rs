...
use coserv_rs::coserv::corim_rs::{Corim, ProfileTypeChoice};
...
impl<'a> CoservParseResult<'a> {
    ...
    pub fn append(&mut self, other: &mut CoservParseResult<'a>) {
        self.rv_list.append(&mut other.rv_list);
        self.ev_list.append(&mut other.ev_list);
        self.evs_list.append(&mut other.evs_list);
    }
    ...
    pub fn update_from_coserv_result<'b>(
        ...
        match coserv_result {
            ...
            ResultSetTypeChoice::EndorsedValues(_) => {
                return Err(Error::custom(
                    "Endorsed Values are present in Coserv result but coserv profile for ARM CCA does not support endorsed values",
                ));
            }
        };
...
impl std::fmt::Debug for CoservParseResult<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = serde_json::to_string_pretty(&self).expect("Failed to serialize CoservParseResult");
...
impl Default for MemCoservStore<'_> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<'a> MemCoservStore<'a> {
    /// Parse Coserv result from bytes and append to make MemCoservStore
    pub fn add_coserv_cbor_bytes(&mut self, bytes: &[u8]) -> Result<()> {
        ...
        self.add_coserv(&coserv)?;
        Ok(())
    }

    /// Parse Coserv object and append to make Endorsement store
    pub fn add_coserv(&mut self, coserv: &Coserv<'a>) -> Result<()> {
...
pub fn parse_coserv<'a, 'b>(coserv: &Coserv<'a>) -> Result<CoservParseResult<'b>> {
    if let Some(coserv_result_set) = &coserv.results {
        ...
        Err(Error::custom("The CoSERV object has empty result set."))
    }
}
...
mod tests {
    ...
    fn test_populate_store_from_coserv_data() {
        ...
        let rv_coserv_bytes = include_bytes!("../test/cbor/rv_result.cbor");
        let ta_coserv_bytes = include_bytes!("../test/cbor/ta_result.cbor");
    ...
    fn test_cover_verify() {
        ...
        let evidence_bytes = include_bytes!("../test/cbor/ccatoken.cbor");
        let rv_coserv_bytes = include_bytes!("../test/cbor/rv_result.cbor");
        let ta_coserv_bytes = include_bytes!("../test/cbor/ta_result.cbor");
